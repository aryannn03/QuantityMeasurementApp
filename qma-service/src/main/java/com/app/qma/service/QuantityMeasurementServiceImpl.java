...
import com.app.qma.quantity.Quantity;
...
public class QuantityMeasurementServiceImpl implements IQuantityMeasurementService {
    ...
    private QuantityDTO convertQuantityToDTO(Quantity<?> quantity) {
        String measurementType = quantity.getUnit()
                .getClass()
                .getSimpleName()
                .replace("Unit", "")
                .toUpperCase();
    ...
    @Override
    @Caching(evict = {
        @CacheEvict(value = "historyCache", key = "'COMPARE' + '-' + #userEmail"),
        @CacheEvict(value = "historyCache", key = "'ALL' + '-' + #userEmail"),
        @CacheEvict(value = "countCache", key = "'COMPARE' + '-' + #userEmail")
    })
    ...
    @Override
    @Caching(evict = {
        @CacheEvict(value = "historyCache", key = "'CONVERT' + '-' + #userEmail"),
        @CacheEvict(value = "historyCache", key = "'ALL' + '-' + #userEmail"),
        @CacheEvict(value = "countCache", key = "'CONVERT' + '-' + #userEmail"),
        @CacheEvict(value = "convertCache", key = "#source.value + '-' + #source.unit + '-' + #targetUnit")
    })
    public QuantityDTO convert(QuantityDTO source, String targetUnit, String userEmail) {
        try {
            ...
            Quantity<?> result = ((Quantity) quantity).convertTo(unit);
    ...
    @Override
    @Caching(evict = {
        @CacheEvict(value = "historyCache", key = "'ADD' + '-' + #userEmail"),
        @CacheEvict(value = "historyCache", key = "'ALL' + '-' + #userEmail"),
        @CacheEvict(value = "countCache", key = "'ADD' + '-' + #userEmail")
    })
    ...
    @Override
    @Caching(evict = {
        @CacheEvict(value = "historyCache", key = "'SUBTRACT' + '-' + #userEmail"),
        @CacheEvict(value = "historyCache", key = "'ALL' + '-' + #userEmail"),
        @CacheEvict(value = "countCache", key = "'SUBTRACT' + '-' + #userEmail")
    })
    ...
    @Override
    @Caching(evict = {
        @CacheEvict(value = "historyCache", key = "'DIVIDE' + '-' + #userEmail"),
        @CacheEvict(value = "historyCache", key = "'ALL' + '-' + #userEmail"),
        @CacheEvict(value = "countCache", key = "'DIVIDE' + '-' + #userEmail")
    })
    ...
    @Override
    @Cacheable(value = "historyCache", key = "#operation + '-' + #userEmail")
    ...
    @Override
    @Cacheable(value = "countCache", key = "#operation + '-' + #userEmail")
    ...
    private void validateSameType(QuantityDTO q1, QuantityDTO q2) {
        if (!q1.getMeasurementType()
                .trim()
                .equalsIgnoreCase(q2.getMeasurementType().trim())) {
